...
import { CanvasElements, EditorCanvasComponent } from '../canvas/editor-canvas.component';
import { EditorToolbarComponent } from '../toolbar/editor-toolbar.component';
import { EditorFooterComponent } from '../footer/editor-footer.component';
...
export class EditorWorkspaceComponent implements OnInit, AfterViewInit, OnDestroy {
  /**
   * Permite reutilizar el workspace en diferentes páginas conservando la misma lógica
   * mientras habilita futuras personalizaciones (por ejemplo, modos de sólo lectura).
   */
  @Input({ required: false }) viewMode: 'home' | 'preview' | 'settings' = 'home';
  ...
  private readonly metadata = inject(AppMetadataService);
  private readonly pendingPdf = inject(PendingPdfService);
  ...
  private annotationsLayer: HTMLDivElement | null = null;
  private pdfViewer: HTMLDivElement | null = null;
  private pdfFileInput: HTMLInputElement | null = null;
  private coordsFileInput: HTMLInputElement | null = null;
  ...
  ngOnInit() {
    const pendingFile = this.pendingPdf.consumeFile();
    if (pendingFile) {
  ...
  onCanvasElementsReady(refs: CanvasElements) {
    this.pdfCanvas = refs.pdfCanvas;
    this.overlayCanvas = refs.overlayCanvas;
    this.annotationsLayer = refs.annotationsLayer;
    this.pdfViewer = refs.pdfViewer;
    void this.render();
    this.redrawAllForPage();
  }

  private syncSidebarInputs() {
    this.pdfFileInput = this.sidebarComponent?.pdfFileInputRef?.nativeElement ?? null;
    this.coordsFileInput = this.sidebarComponent?.coordsFileInputRef?.nativeElement ?? null;
  }
  ...
  onDocumentMouseDown(event: MouseEvent) {
    ...
    if (!this.isTargetInside(target, '.annotation-editor--editing')) {
      this.cancelEdit();
    }
  }
  ...
  private isTargetInside(target: Element | null, selector: string) {
    if (!target) {
      return false;
    }
  ...
  pasteAnnotation(targetPageNum?: number): boolean {
    ...
    const pdfCanvas = this.pdfCanvas ?? null;
  ...
  triggerImportCoords() {
    this.syncSidebarInputs();
    const input = this.coordsFileInput ?? this.ensureCoordsFileInput();
    if (!input) {
